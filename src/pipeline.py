...
* ``NonsenseGenerator.build`` now receives ``attn_implementation``.
* Output paths come from ``cfg.paths`` (already resolved to a writable
  location by ``config.load_config``), so no stage writes into
  ``/kaggle/input/``.
* ``generate_vocabulary`` failure is logged but does not abort — a tiny
  vocabulary is acceptable because ``attacks.json`` is generated from the
  nonsense model, not the word pool.
"""
...
def build_submission(cfg: Config) -> pd.DataFrame:
    ...
    log.info("Loaded %d test rows from %s", len(test_df), cfg.paths.test_csv)

    bnb = build_nf4_config()
    attn = cfg.evaluation.attn_implementation or "eager"

    # ---- 1. Vocabulary ----------------------------------------------------
    if cfg.paths.words_json.exists():
        words = json.loads(Path(cfg.paths.words_json).read_text())
        log.info("Loaded %d cached vocabulary words", len(words))
    else:
    ...
    gen = NonsenseGenerator.build(
        cfg.generation.nonsense, bnb, cfg.hf_token, attn_implementation=attn
    )
...
__all__ = ["build_submission", "evaluate_submission"]